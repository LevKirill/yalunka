...
(_a = document.querySelector(".cart-checkout")) == null ? void 0 : _a.addEventListener("click", (e) => {
  e.preventDefault();
...
if (orderConfirmBtn) {
  orderConfirmBtn.addEventListener("click", (e) => {
    ...
    if (!isValid) {
      showNotification("Будь ласка, заповніть усі обов'язкові поля!", true);
      return;
    }
    showNotification("Дякуємо! Ваше замовлення прийнято.");
    orderPopup.classList.remove("open");
    orderForm.reset();
  });
}
document.querySelectorAll(".buy-click").forEach((btn) => {
  btn.addEventListener("click", (e) => {
    ...
    const title = ((_a2 = productContainer.querySelector(".product__title")) == null ? void 0 : _a2.textContent) || "Товар";
    const price = parseFloat(
      ((_b = productContainer.querySelector(".product__price span")) == null ? void 0 : _b.textContent.replace(/\s/g, "")) || "0"
    );
    const image = ((_c = productContainer.querySelector(".product__main img")) == null ? void 0 : _c.src) || "";
    const quickCart = [
      {
        id: "quick_" + Date.now(),
        title,
